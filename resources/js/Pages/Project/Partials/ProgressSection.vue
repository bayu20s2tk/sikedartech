...
<template>
    <ActionSection v-if="props.project.worker_id == $page.props.user.id">
        ...
        <template #content>
            ...
            <InputError :message="form.errors.range" class="mt-2"/>
              ...
              <span v-if="activityItemIdx !== props.project.progress.length - 1"
                    class="absolute top-5 left-5 -ml-px h-full w-0.5 bg-gray-300" aria-hidden="true"/>
                            <div class="relative flex items-start space-x-3">
                                <div class="relative">
                                    <img
                                        class="flex h-10 w-10 items-center justify-center rounded-full bg-gray-400 ring-1 ring-gray-300"
                                ...
                                <div class="min-w-0 flex-1">
                                    <div>
                                        ...
                                        <p class="mt-0.5 text-sm text-gray-500">
                                            {{ formattedDate(activityItem.created_at) }}</p>
                                    </div>
                                    ...
                                    <div class="w-32 mt-2">
                                        ...
                                        <div class="w-full bg-gray-200 rounded-full h-2.5">
                                            <div class="bg-primary-700 h-2.5 rounded-full"
                                                 :style="{ 'width': activityItem.range + '%' }"/>
                                        </div>
                                    </div>
                                </div>
                            </div>
